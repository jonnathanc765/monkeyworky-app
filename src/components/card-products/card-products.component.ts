...
export default defineComponent({
    ...
    setup(props: Data, context: SetupContext) {
        ...
        const deleteProduct = async (id: number) => {
            if (auth.value.role) {
                if (auth.value.role === 'admin') {
                    disabled.value = true;
                    alertConfirmationBulma('warning', 'Eliminar producto', '¿Está seguro que desea eliminar este producto?', async () => {
                        await apiDelete(id);
                    }, () => {
        ...
        const apiDelete = async (id: number) => {
            await productStore.dispatch('delete', id).then(() => {
                alertBulma('warning', 'Producto eliminado', 'El producto fue eliminado satisfactoriamente');
            }).catch(() => {
                alertBulma('warning', 'Error', 'Hubo un problema a la hora de eliminar el producto, intente nuevamente');
            }).finally(() => {
                disabled.value = false;
            });
        };
