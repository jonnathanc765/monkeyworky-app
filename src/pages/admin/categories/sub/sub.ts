  ...
  setup() {
    ...
    const get = async () => {
      await categoriesStore
        .dispatch(
          'getSubCategories',
          page.value !== 0 ? { pagination: { page: page.value } } : ''
        )
        .finally(() => {
    ...
    const deleteSub = (row: any) => {
      ...
      alertConfirmationBulma(
        ...
        `¿Está seguro que desea eliminar la sub categoría ${row.name}?`,
        async () => {
          await categoriesStore
            .dispatch('deleteSub', row.sub_category_id)
            .then(() => {
              alertBulma(
                'warning',
                'Sub categoría eliminada',
                'Se eliminó la sub categoría satisfactoriamente'
              );
            })
            .catch(() => {
              alertBulma(
                'warning',
                'Error',
                'Hubo un problema a la hora de comunicarse con el servidor'
              );
            })
            .finally(() => {
