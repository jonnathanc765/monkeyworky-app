  ...
  setup(props: { item: any }, context: SetupContext) {
    ...
    const action = async () => {
      ...
      await categoriesStore
        .dispatch(
          props.item.id ? 'updateCategory' : 'postCategory',
          props.item.id ? { data: formData, id: props.item.id } : formData
        )
        .then((resp) => {
          console.log(resp);
          dismiss();
          alertBulma(
            ...
            props.item.id
              ? 'Se actualizó la categoría satisfactoriamente'
              : 'Se añadió la categoría satisfactoriamente'
          );
        })
        .catch((error) => {
          if (error.status === 422) {
