  ...
  setup() {
      ...
      await bankStore
        ...
        .catch(() => {
          alertBulma(
            'warning',
            'Error',
            'No se pudo cargar el listado de bancos, intente nuevamente'
          );
        })
        .finally(() => {
    ...
    const deleteBank = async (row: any) => {
      ...
      alertConfirmationBulma(
        'warning',
        'Eliminar banco',
        `¿Está seguro que desea eliminar el banco ${row.name}?`,
        async () => {
          bankStore
            .dispatch('delete', row.bank_id)
            .catch(() => {
              alertBulma(
                'warning',
                'Error',
                'Hubo un problema a la hora de eliminar el banco'
              );
            })
            .finally(() => {
