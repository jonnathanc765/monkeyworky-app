...
export default {
    ...
    setup(props: { modal: boolean }) {
        ...
        const getOrders = async (value?: any) => {
            await orderStore.dispatch('getOrders', (searchStatus.value !== 'all' ? { filter: { status: searchStatus.value } } : pagination.value ? { pagination: { page: value } } : '')).catch((error) => {
                console.log(error);
                alertBulma('warning', 'Error', 'No se pudieron cargar el listado de ordenes, por favor intente recargar la página', { label: 'Entendido' });
        ...
        return { orders, title, dateParse, caseStatus, openModal, itemId, itemValue, pagination, actionPagination, isActive, filterStatus, selectStats, route, auth, isChange, row };
    },
};
