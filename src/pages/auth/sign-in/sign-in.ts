  ...
  setup() {
    ...
    const registerProducts = () => {
      if (productsLocal.value.length > 0) {
        cartStore
          .dispatch('addCartAuth')
          .catch((error) => {
            console.log(error);
            alertBulma(
              'warning',
              'Error',
              'No pudimos registrar algunis items del carrito de compra en la base de datos, la comunicación falló'
            );
          })
          .finally(() => {
      ...
      await authStore
        ...
        .catch((error: any) => {
          ...
          alertBulma(
            'warning',
            'Error fatal',
            'Tuvimos problemas con el servidor, por favor intenta en un momento.',
            { label: 'Entendido' }
          );
        });
    };
