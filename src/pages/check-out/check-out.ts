...
export default {
  components: {
    Products: defineAsyncComponent(() => import('./products/products.vue')),
    Info: defineAsyncComponent(() => import('./info/info.vue')),
    Address: defineAsyncComponent(() => import('./address/address.vue')),
    Pays: defineAsyncComponent(() => import('./pays/pays.vue')),
  ...
  setup() {
    ...
    const checkEmptyPayment = () => {
      if (
        ...
        checkOut.state.registerPay.date !== '' &&
        checkOut.state.registerPay.bankDestination
      ) {
        return true;
      }
      return false;
      ...
      await checkOut
        ...
        .then((res) => {
          ...
          alertConfirmationBulma(
            'warning',
            'Redirigiendo a Whatsappp',
            'Se agregó satisfactoriamente el pedido, deseas enviar el pedido a Whatsapp?',
            () => {
        ...
        .catch((error) => {
          ...
          if (error.status === 422) {
            ...
            alertBulma(
              ...
              `La dirección suministrada no fue añadida, vuelva al paso 3 y presione en "Añadir nueva dirección"`
            );
            return;
            ...
            alertBulma(
              ...
              `Para poder realizar una orden primero debe rellenar todos los datos en el perfil`
            );
            return;
          }
          alertBulma(
            'warning',
            'Error',
            'Hubo un problema a la hora de procesar su orden, por favor vuelva a intentarlo'
          );
        });
    };
      ...
      await checkOut
        ...
        .catch(() => {
          ...
          alertBulma(
            'warning',
            'Error',
            'La orden fue generada satisfactoriamente, pero hubo un problema con el registro del pago, por favor registre el pago nuevamente.',
            { label: 'Entendido' }
          );
