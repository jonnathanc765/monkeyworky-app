    ...
    setup(props: { amount: number, isModal: boolean }) {
        ...
        onMounted(() => {
            ...
            bankStore.dispatch('get').catch(() => {
                alertBulma('warning', 'Error al cargar los bancos', 'Hubo un problema en la comunicación del servidor y no pudimos cargar los bancos, por favor recarga la página');
            });
        });
        ...
        const banksDestination = computed(() => {
            if (!isBs.value) {
                registerPay.value.bankDestination = bankStore.state.bankDestinationBs[0];
            }
        ...
        const total = computed(() => {
            if (!props.isModal) {
                registerPay.value.amount = isBs.value ? cartStore.state.total : cartStore.state.subtotal;
        ...
        const select = (event: any) => {
            if (event.target.value !== 'Seleccione') {
                const bank = banks.value.find((res) => res.bank_id === Number(event.target.value));
                if (bank) {
        ...
        const getImage = (event: any) => {
            if (event.target.files[0].size > 6556410) {
                alertBulma('danger', 'Error', 'El comprobante de pago no puede pesar más de 6MB');
            } else if (event.target.files[0].type !== 'image/jpg' && event.target.files[0].type !== 'image/png' && event.target.files[0].type !== 'image/jpeg') {
                alertBulma('danger', 'Error', 'El comprobante de pago debe tener un formato .JPG, .PNG o .pdf');
            } else {
        ...
        return { registerPay, select, addForm, total, getImage, banks, banksDestination, selectDestination, isBs, bankDestinationBs, truncate };
    },
};
