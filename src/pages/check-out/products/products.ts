...
export default {
    ...
    setup(props: any, context: SetupContext) {
        ...
        const textValues = (row: any, index: number) => {
            switch (index) {
                case 0:
                    return (row.variation) ? `${row.product.name} (${row.variation.size})` : `${row.product.name} (${row.size})`;
                case 1:
                    return row.quantity;
                case 2:
        ...
        const deleteItem = async (id: number, cartId: number) => {
            ...
            if (authStore.getters.isAuth) {
                await cartStore.dispatch('deleteCartApi', { variationId: id, id: cartId }).catch((error: any) => {
                    deleteFailed();
                    console.log(error);
                }).finally(() => {
                    disabled.value[id] = 0;
        ...
        const deleteFailed = () => {
            alertBulma('danger', 'Error', 'No pudimos eliminar el artículo, vuelva a intentarlo.', { label: 'Entendido' });
        };

        return { textValues, textProuct, deleteItem, disabled };
    },
};
