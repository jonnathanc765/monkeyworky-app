...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            addressStore.dispatch('get').catch(() => {
                alertBulma('danger', 'Error', 'Hubo un problema con la comunicación del servidor a la hora de cargar las direcciones, por favor recarga la página');
        ...
        const select = (event: any) => {
            const values = addresses.value.find((res: any) => res.address_id === Number(event.target.value));
            if (values) {
