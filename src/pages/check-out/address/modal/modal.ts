...
export default {
    ...
    setup(props: any, context: SetupContext) {
        ...
        const getMunicipality = async (id: number) => {
            ...
            await addressStore.dispatch('getMunicipality', id).then((res) => {
                municipality.value = res[0].id;
            }).catch(() => {
                alertBulma('danger', 'Error', 'No se pudo cargar los municipios');
            }).finally(() => {
        ...
        onMounted(async () => {
            await addressStore.dispatch('getStates').catch(() => {
                alertBulma('danger', 'Error', 'Hubo un problema a la hora de cargar los estados');
        ...
        const getParishes = async (id: number) => {
            ...
            addressStore.commit('setParishes', []);
            await addressStore.dispatch('getParishes', id).then((res) => {
                parish.value = res[0].id;
            }).catch(() => {
                alertBulma('danger', 'Error', 'No se pudo cargar las parroquias');
            }).finally(() => {
        ...
        const addAddress = () => {
            ...
            addressStore.dispatch('post', { parish: parish.value, ...form.value }).then((res) => {
                ...
                alertBulma('warning', 'Registro de dirección', 'No se pudo registrar la nueva dirección, intente nuevamente');
            }).finally(() => {
        ...
        return { dismiss, auth, changeState, states, isLoadingState, isLoadingMun, state, municipalities, parishes, changeMunicipality, isLoadingParishes, changeParish, addAddress, form, disabled, arrow, arrowVisible, eventScroll };
    },
};
