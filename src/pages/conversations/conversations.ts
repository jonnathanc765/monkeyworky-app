  ...
  setup() {
      ...
      await conversationsStore
        ...
        .catch(() => {
          alertBulma(
            'warning',
            'Error',
            'Hubo un problema con la comunicación del servidor'
          );
        })
        .finally(() => {
    ...
    const showMessages = async (row: any) => {
      ...
      if (row.id !== idConversation.value) {
        ...
        textConversation.value.name =
          row.from.people_id !== auth.value.people.people_id
            ? row.from.firstname + ' ' + row.from.lastname
            : row.to.firstname + ' ' + row.to.lastname;
        textConversation.value.id =
          row.from.people_id !== auth.value.people.people_id
            ? row.from.people_id
            : row.to.people_id;
    ...
    const getPeople = async () => {
      ...
      await authStore
        .dispatch('getUsers', {
          role: auth.value.role === 'customer' ? 'admin' : 'customer',
          page: page.value,
        })
        .catch(() => {
          alertBulma(
    ...
    const selectList = async (row: any) => {
      if (!isActiveMessage.value) {
        await conversationsStore
          .dispatch('postConversation', row.people_id)
          .then(async (res) => {
            list.value = false;
            conversationsStore.commit('setPaginate', 1);
            pageMessage.value = 2;
    ...
    const getMessages = async (id: number, scroll = false) => {
      ...
      await conversationsStore
        ...
        .catch(() => {
          alertBulma('warning', 'Error', 'No se pudo cargar los mensajes');
        })
        .finally(() => {
          isActiveMessage.value = false;
        });
      ...
      if (text) {
        ...
        if (message !== '') {
          ...
          await conversationsStore
            ...
            .catch(() => {
              alertBulma('warning', 'Error', 'No se pudo mandar el mensaje');
            })
            .finally(() => {
              scrollHeight();
            });
